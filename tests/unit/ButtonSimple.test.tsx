...
describe('Button Component Simple Tests', () => {
  ...
  it('handles press events', () => {
    ...
    act(() => {
      instance = renderer.create(<Button title="Press Me" onPress={onPressMock} />);
    });
  ...
  it('respects disabled state for press events', () => {
    const onPressMock = jest.fn();
    let instance;
    
    act(() => {
      instance = renderer.create(<Button title="Press Me" onPress={onPressMock} disabled={true} />);
    });
    
    const button = instance.root.findByProps({ testID: 'button' });
    
    // Attempt to call onPress directly, but in a real component the disabled state 
